...
def iter_nonrandom_filled_containers(area_dir: pathlib.Path):
...
def main() -> None:
    ...
    with OUTPUT_TPA.open("w") as fh:
        fh.write("// Auto-generated by staging/scripts/nonrandom_container_assignments.py\n")
        fh.write("// Adds one flavored random placeholder to non-empty containers without RND entries.\n\n")
    ...
    with OUTPUT_MD.open("w") as fh:
        fh.write("# Nonrandom Filled Container Assignments\n\n")
        fh.write(f"- Containers assigned: `{len(assignments)}`\n")
